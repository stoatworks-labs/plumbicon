...
(macOS) + Windows `.dll`. MIT. Public at github.com/stoatworks-labs/plumbicon,
released at v0.1.0 on 2026-09-23. 24 parameters (19 controls + a five-entry
About block). User guide: `docs/USER-GUIDE.md` (the only copy anyone edits),
rendered to `docs/USER-GUIDE.pdf` and
https://stoatworks-labs.com/software/plumbicon/guide/ by the website's
`build_guides.py`.
...
- **A tube type is an OVERRIDE, not a write.** `Effective()` is the one place
  ...
  `tools/sweep.py` gives every one of them a `Type=0` context. Their defaults
  are the Plumbicon row, so switching to `Custom` changes nothing (two were
  not until 2026-09-23 — `4d1fd7f`).
...
- `source/StoatworksAbout.h` is GENERATED by stoatworks-backend's
  ...
  is registered there and in the website's `projects.json`. Never hand-edit
  either file.
...
## Not done yet
- **Never loaded into Resolume on macOS.** Everything here was compiled,
