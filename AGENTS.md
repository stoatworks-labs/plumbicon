...
and a Windows `.dll`. MIT, public at `github.com/stoatworks-labs/plumbicon`
(created 2026-09-23), released at v0.1.0 on 2026-09-23. The user guide is
`docs/USER-GUIDE.md` — the only copy anyone edits — rendered to
`docs/USER-GUIDE.pdf` and to https://stoatworks-labs.com/software/plumbicon/guide/
by the website's `build_guides.py`. It is registered in the website's
`projects.json` (status beta, guide true) and in stoatworks-backend's
`sync-about.py` TARGETS, `names.json`, `visibility.json` and `derived.json`;
`source/StoatworksAbout.h` is generated by `sync-about.py` and
`ATTRIBUTIONS.md` by `sync-attributions.py`, so edit neither by hand.
...
- **It loads in a real FFGL host.** `oxbow selftest` instantiates it and renders
  **120 frames, gl error 0x0, PASS**, reporting `SW Plumbicon` / `PB01` /
  ...
  Re-run 2026-09-23 after registration: **24** parameters (the About block
  now has five entries — About text, User guide, Project page, Source on
  GitHub, Support the work), still six groups, 120 frames, PASS.
- **CI has run on GitHub.** `ci.yml` on the macOS runner, which has no GPU:
  the harness falls back to Apple's software renderer, and the five physics
  checks passed there as a gate once the `--capacity` tolerance was fixed
  (below). The Windows x64 DLL has been compiled with MSVC by `release.yml`.
...
- **Never loaded into Resolume on macOS.** On Windows, a CI build passed Resolume Arena 7.27.1's gate on llvmpipe on 2026-09-23 (8 of 9 checks; Recovery and Burn Rate read dead on the gate's still picture — see README Status). Everything here was
...
- **The universal build has never run on an Intel Mac.** `lipo` says the
  slice is there; nothing has executed it.
...
and the sweep encodes. The defaults of those eight are the Plumbicon row, so
switching from the default Plumbicon to Custom changes nothing. Until
2026-09-23 two were not (Target Capacity 0.865 against 0.45, Lag Amount 0.18
against 0.22) and Custom jumped the tail from about two fields to about five;
found writing the user guide, fixed in `4d1fd7f`.
